...
<div class="max-w-5xl mx-auto p-6">
    ...
    <form action="{{ route('superadmin.form-builder.store') }}" method="POST" class="bg-white p-6 rounded-3xl border border-slate-100 shadow-3xs mb-8">
        ...
        <div class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
                ...
                <div class="space-y-1.5 md:col-span-3">
                    ...
                    <select name="type" id="typeSelector" class="w-full p-3 border border-slate-200 rounded-xl text-xs bg-slate-50/50 focus:bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none" onchange="toggleOptions(this.value)">
                        ...
                        <option value="number">Angka (Number Input)</option>
                        <option value="date">Tanggal (Date Input)</option>
                        <option value="file">File / Dokumen (File Upload)</option>
                ...
                <div id="optionsWrapper" class="space-y-1.5 md:col-span-3 hidden">
                    ...
                    <input type="text" name="options" id="optionsInput" placeholder="S, M, L, XL (pisahkan dengan koma)" class="w-full p-3 border border-slate-200 rounded-xl text-xs bg-slate-50/50 focus:bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none">
                </div>
            ...
            <!-- New fields for Description and Example -->
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase text-slate-500 tracking-wide">Panduan / Petunjuk Pengisian</label>
                    <input type="text" name="description" placeholder="Contoh: Harap unggah scan KTP berwarna format PDF ukuran maksimal 2MB." class="w-full p-3 border border-slate-200 rounded-xl text-xs bg-slate-50/50 focus:bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none">
                </div>

                <div class="space-y-1.5">
                    <label class="block text-xs font-bold uppercase text-slate-500 tracking-wide">Contoh Pengisian (Placeholder)</label>
                    <input type="text" name="example" placeholder="Contoh: 1234567890123456 atau L" class="w-full p-3 border border-slate-200 rounded-xl text-xs bg-slate-50/50 focus:bg-white focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none">
                </div>
            </div>
